...
import { getFfmpegClient } from "../../lib/ffmpeg/ffmpegClient";
...
export function VideoTrimmer({ file, onTrimmed, onClose }: VideoTrimmerProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
      <div className="bg-neutral-900 p-6 rounded-lg w-2xl text-white shadow-xl">
        ...
        {end !== null && (
          <>
            <div className="relative mt-6 mb-3 h-4" ref={barRef}>
              <div className="absolute top-1/2 -translate-y-1/2 w-full h-1.5g-slate-700 rounded-full" />
              ...
              <div
                className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-lime-400 border-2 border-white rounded-full cursor-pointer"
                style={{
                  left: `${(start / duration) * 100}%`,
        ...
        <div className="flex justify-end gap-3 mt-5">
          <button onClick={onClose} className="px-4 py-2 bg-slate-700 rounded-md hover:bg-slate-600">
            Cancel
          </button>
          <button
            onClick={handleTrim}
